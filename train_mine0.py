...
from model_mine0 import embed_net
from utils import *
from loss import CrossEntropyLabelSmooth, CenterDCLoss, CenterClusterCompactLoss
...
parser.add_argument('--model_path', default='save_model5/', type=str,
...
criterion_C3 = CenterClusterCompactLoss()
criterion_C3.to(device)

criterion_CDC = CenterDCLoss()
criterion_CDC.to(device)
...
def train(epoch):
    ...
    c3_loss = AverageMeter()
    cdc_loss = AverageMeter()
    ...
    for batch_idx, (input10, input11, input2, label1, label2) in enumerate(trainloader):
        ...
        feat, out0, feat_all, out1 = net(input1, input2)

        loss_id = criterion_id(out0[0], labels) + criterion_id(out1, labels)
        loss_c3 = criterion_C3(feat[0], labels)
        loss_cdc = criterion_CDC(out0[0], labels)
        for i in range(len(feat) - 1):
            loss_id += criterion_id(out0[i + 1], labels)
            loss_c3 += criterion_C3(feat[i + 1], labels)
            loss_cdc += criterion_CDC(out0[i + 1], labels)
        loss_c3 += criterion_C3(feat_all, labels)

        loss = loss_id + 0.5 * loss_c3 + 0.5 * loss_cdc
        ...
        c3_loss.update(loss_c3, 2 * input1.size(0))
        cdc_loss.update(loss_cdc, 2 * input1.size(0))
        total += labels.size(0)
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                ...
                train_loss=train_loss, id_loss=id_loss, tri_loss=c3_loss, metric_loss=cdc_loss))
