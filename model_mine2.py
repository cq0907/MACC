...
class embed_net(nn.Module):
    def __init__(self, class_num, no_local='off', gm_pool='on', arch='resnet50', share_net=1, pcb='on',
        ...
        if self.pcb == 'on':
            ...
            self.blocks = nn.ModuleList([copy.deepcopy(block) for _ in range(6)])
        ...
        if self.pcb == 'on':
            ...
            for i in range(self.num_stripes):
                ...
                if self.gm_pool == 'on':
                    ...
                    local_feat = self.blocks[i](local_feat)
                    b, c, h, w = local_feat.shape
                    local_feat = local_feat.view(b, c, -1)
                    local_feat = (torch.mean(local_feat ** self.p, dim=-1) + 1e-12) ** (1 / self.p) # regDB: 10.0    SYSU: 3.0
                else:
                    # average pool
                    # local_feat = F.avg_pool2d(feat[:, :, i * stripe_h: (i + 1) * stripe_h, :],(stripe_h, feat.size(-1)))
                    local_feat = F.max_pool2d(feat[:, :, i * stripe_h: (i + 1) * stripe_h, :],
                                              (stripe_h, feat.size(-1)))
                local_feat = local_feat.view(feat.size(0), feat.size(1), 1, 1)
                # local_feat = torch.cat([local_feat.view(feat.size(0), feat.size(1), 1, 1), mul_layer_feats], dim=1)
                # # shape [N, c, 1, 1]
            ...
            if self.training:
                return local_feat_list, logits_list, feat_all, x_pool, self.classifier(feat)
            else:
