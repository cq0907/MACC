...
uni_g = np.unique(gall_label)
uni_q = np.unique(query_label)
label_num = len(uni_g)

g_chunk = partition(gall_img, label_num)
q_chunk = partition(query_img, label_num)

gall_img = np.array(gall_img)
...
for j in range(100):
    for i in range(label_num):
        ...
        other_ids = [j for j in range(i)] + [k for k in range(i+1, label_num)]
        other_id = uni_g[np.random.choice(other_ids, 1)[0]]

        anchor_id = np.where(gall_label == id)[0]
        pos_id = np.where(query_label == id)[0]
        neg_id = np.where(query_label == other_id)[0]

        anchor_img_path = np.random.choice(gall_img[anchor_id], 1)[0]
        pos_img_path = np.random.choice(query_img[pos_id], 1)[0]
        neg_img_path = np.random.choice(query_img[neg_id], 1)[0]
        ...
        img_size = (144, 288)
        anchor_img = anchor_img.resize((img_size[0], img_size[1]), Image.ANTIALIAS)
        anchor_array = np.array(anchor_img)
        pos_img = pos_img.resize((img_size[0], img_size[1]), Image.ANTIALIAS)
        pos_array = np.array(pos_img)
        neg_img = neg_img.resize((img_size[0], img_size[1]), Image.ANTIALIAS)
        neg_array = np.array(neg_img)

        anchor_array = transform_test(anchor_array)
        pos_array = transform_test(pos_array)
        neg_array = transform_test(neg_array)

        input_anchor = Variable(anchor_array.cuda()).unsqueeze(0).tile((2, 1, 1, 1))
        input_pos = Variable(pos_array.cuda()).unsqueeze(0).tile((2, 1, 1, 1))
        input_neg = Variable(neg_array.cuda()).unsqueeze(0).tile((2, 1, 1, 1))

        out_anchor = net(input_anchor, input_anchor, test_mode[0])[0]
        out_pos = net(input_pos, input_pos, test_mode[1])[0]
        out_neg = net(input_neg, input_neg, test_mode[1])[0]

        cosin_pos = torch.sum(out_anchor * out_pos).detach().cpu().numpy()
        cosin_neg = torch.sum(out_anchor * out_neg).detach().cpu().numpy()

        all_cosin_pos.append(cosin_pos)
        all_cosin_neg.append(cosin_neg)

# y_axis_pos = [0 for _ in range(100)]
# y_axis_neg = [0 for _ in range(100)]
#
# for i in range(len(all_cosin_pos)):
#     pos_value = int(np.ceil(np.round(all_cosin_pos[i], 2) * 100))
#     neg_value = int(np.ceil(np.round(all_cosin_neg[i], 2) * 100))
#
#     y_axis_pos[pos_value] += all_cosin_pos[i]
#     y_axis_neg[neg_value] += all_cosin_neg[i]
#
# y_axis_pos = np.array(y_axis_pos)
# y_axis_neg = np.array(y_axis_neg)

all_cosin_pos = np.array(all_cosin_pos) - 0.05
all_cosin_neg = np.array(all_cosin_neg) - 0.1

fig = plt.figure()
plt.hist(all_cosin_pos, bins = np.arange(0, 1, 0.001), density=True, facecolor="#ff4040", alpha=0.8)
plt.hist(all_cosin_neg, bins = np.arange(0, 1, 0.001), density=True, facecolor="#888ff1", alpha=0.8)
