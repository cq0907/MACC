...
from model_mine2 import embed_net
from utils import *
from loss import CenterTripletLoss, CrossEntropyLabelSmooth, TripletLoss_WRT, CenterWeightTripletLoss, TripletLoss_ADP, JSDivLoss, DivLoss
...
parser.add_argument('--log_path', default='log1/', type=str,
                    help='log save path')
...
if args.method=='agw':
    ...
    suffix = suffix + '_base_gm10_k{}_p{}_lr_{}_seed_{}_6'.format(args.num_pos, args.batch_size, args.lr, args.seed)
...
if args.method =='base':
    net = embed_net(n_class, no_local= 'off', gm_pool = 'on', arch=args.arch, share_net=args.share_net, pcb=args.pcb, local_feat_dim=args.local_feat_dim, num_strips=args.num_strips, p=args.p)
else:
    ...
    for batch_idx, (input10, input11, input2, label1, label2) in enumerate(trainloader):
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'MLoss: {metric_loss.val:.4f} ({metric_loss.avg:.4f}) '
                  'Accu: {:.2f}'.format(
...
for epoch in range(start_epoch, 61 - start_epoch):
    ...
    if epoch > 0 and epoch % 2 == 0:
        ...
        # save model
        if cmc_fc[0] > best_rank1 or mAP_fc > best_mAP or mINP_fc > best_mINP:  # not the real best for sysu-mm01
